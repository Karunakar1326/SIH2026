        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded bg-accent text-white hover:bg-accent-dark transition-colors shadow-2xs"
            >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 px-6 py-4">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border border-neutral-300 rounded-lg p-4 shadow-2xs">
              <h3 className="text-xs font-bold text-neutral-800 uppercase tracking-wider mb-3">Settlement Location & Attributes</h3>
              <div className="grid grid-cols-2 gap-3 text-xs">
                {[
                  { icon: <MapPin size={12} />, label: 'GPS Coords', value: `${hab.coordinates.lat}°, ${hab.coordinates.lng}°` },
                  { icon: <Users size={12} />, label: 'Population', value: formatNumber(hab.population) },
                  { icon: <Home size={12} />, label: 'Households', value: formatNumber(hab.households) },
                  { icon: <Layers size={12} />, label: 'Land Area', value: `${hab.area_sqkm} km²` },
                  { icon: <Mountain size={12} />, label: 'Elevation MSL', value: `${hab.elevation_m} m` },
                  { icon: <Users size={12} />, label: 'Pop Density', value: `${hab.population_density}/km²` },
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-2 bg-neutral-50 p-2 rounded">
                    <span className="text-neutral-500 mt-0.5">{item.icon}</span>
                    <div>
                      <div className="text-[9px] text-neutral-500">{item.label}</div>
                      <div className="text-xs font-bold text-neutral-900">{item.value}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white border border-neutral-300 rounded-lg p-4 shadow-2xs">
              <h3 className="text-xs font-bold text-neutral-800 uppercase tracking-wider mb-3">Red-Zone Classification Analysis</h3>
              <RedZoneBadge redZone={hab.red_zone} size="lg" />
              <p className="text-xs text-neutral-700 font-medium mt-3 leading-relaxed bg-neutral-50 p-2.5 rounded border border-neutral-200">
            ...
            <div className="bg-white border border-neutral-300 rounded-lg p-4 shadow-2xs">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-xs font-bold text-neutral-800 uppercase tracking-wider">Relocation Priority Index (RPI)</h3>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${urgencyColor(hab.relocation_urgency)} uppercase`}>
                  {urgencyLabel(hab.relocation_urgency)}
                </span>
              </div>
              <div className="flex items-baseline gap-2 mb-3">
                <span className="text-3xl font-extrabold text-neutral-900">{hab.relocation_priority}</span>
                <span className="text-xs text-neutral-400 font-bold">/100 RPI Score</span>
              </div>
                ...
                {[
                  ...
                  <div key={i}>
                    <div className="flex justify-between text-[11px] mb-0.5">
                      ...
                      <span className="font-bold text-neutral-800">{item.score}/{item.max}</span>
                    </div>
                    <div className="h-1.5 bg-neutral-100 rounded-full overflow-hidden">
                      <div className="h-full bg-accent rounded-full" style={{ width: `${(item.score / item.max) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border border-neutral-300 rounded-lg p-4 shadow-2xs">
              <h3 className="text-xs font-bold text-neutral-800 uppercase tracking-wider mb-3">Multi-Hazard Score Profile</h3>
            ...
            {/* Vulnerability Index */}
            <div className="bg-white border border-neutral-300 rounded-lg p-4 shadow-2xs">
              <h3 className="text-xs font-bold text-neutral-800 uppercase tracking-wider mb-3">Vulnerability Sub-Indicators</h3>
              <div className="space-y-2.5">
          ...
          <div className="space-y-4">
            {/* GIS Map Location */}
            <div className="bg-white border border-neutral-300 rounded-lg overflow-hidden h-64 shadow-2xs">
              <RiskMap
            ...
            {assignment ? (
              ...
              <div className="bg-white border border-neutral-300 rounded-lg p-4 text-xs text-neutral-500">
